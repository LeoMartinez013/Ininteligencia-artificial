...
print(neuronio, "saida =", saida_test, "->", "Perdedor" if saida_test == 0 else "Vencedor")
# ============================================================================ 
# Plotagem do gráfico do teste de generalização
# ============================================================================ 

# Pontos de treinamento (já definidos anteriormente)
# Classe 0: jogadores Perdedores (padrao0 a padrao5)
eliminacoes_class0 = [padrao0[1], padrao1[1], padrao2[1], padrao3[1], padrao4[1], padrao5[1]]
mortes_class0 = [padrao0[2], padrao1[2], padrao2[2], padrao3[2], padrao4[2], padrao5[2]]
...
# Definição dos pontos de generalização (novos dados)
p0 = [-1, 0.3, 0.7]    # Expectativa: Perde
p1 = [-1, 0.35, 0.65]  # Expectativa: Perde
p2 = [-1, 0.8, 0.4]    # Expectativa: Vence
p3 = [-1, 1.0, 0.2]    # Expectativa: Vence
p4 = [-1, 0.5, 0.6]    # Expectativa: Perde
pontos_generalizacao = [p0, p1, p2, p3, p4]

# Extrai as coordenadas dos pontos de generalização
gen_eliminacoes = [p[1] for p in pontos_generalizacao]
gen_mortes = [p[2] for p in pontos_generalizacao]

# Determina as classes previstas para cada ponto de generalização (para definir a cor)
gen_cores = []
for p in pontos_generalizacao:
    _, saida_test = teste_generalizacao(neuronio, p, saida0)
    # Se a saída for 0, classifica como perdedor (azul), se for 1, como vencedor (vermelho)
    gen_cores.append('blue' if saida_test == 0 else 'red')

# Cria o gráfico (baseado no gráfico de treinamento)
plt.figure(figsize=(8, 6))
plt.scatter(eliminacoes_class0, mortes_class0, color='blue', label='Treino - Perdedores')
plt.scatter(eliminacoes_class1, mortes_class1, color='red', label='Treino - Vencedores')

# Calcula e plota a fronteira de decisão:
# -w0 + w1*Eliminações + w2*Mortes = 0  =>  Mortes = (w0 - w1*Eliminações) / w2
w0, w1, w2 = neuronio
elim_range = np.linspace(0, 1, 100)
mortes_boundary = (w0 - w1 * elim_range) / w2
...
# Plota os pontos de generalização com marcador 'x' e tamanho aumentado
plt.scatter(gen_eliminacoes, gen_mortes, color=gen_cores, marker='x', s=100, label='Generalização')

plt.xlabel("Eliminações (normalizado)")
plt.ylabel("Mortes (normalizado)")
plt.title("Teste de Generalização - Marvel Rivals (Valores Normalizados)")
plt.xlim(0, 1)
plt.ylim(0, 1)
plt.legend()
plt.grid(True)
